...
class Global_data extends CI_Model{
	...
	function WAbizteks($pesan,$hp){
   		$hp = '6288801219155';
		$pesan = 'Tess Nya ieu Uyyy';
		$key = 'b5bc5bbe1f'; //ganti api key anda
        $id = '166'; // ID Device Anda
        $hp = $hp.'@s.whatsapp.net';
        $url_app = "http:/notifwa.adam.biz.id";
        $data = array(
            'id_device' => $id,
            'message' => $pesan,
            'tujuan' => $hp
        ...
        curl_setopt($curl, CURLOPT_URL, $url_app."/api/sendText/?".$params);
        curl_setopt($curl, CURLOPT_RETURNTRANSFER, true);
        ...
        var_dump(
            [
                gethostbyname('notifwa.adam.biz.id'),
