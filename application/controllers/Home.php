...
class Home extends CI_Controller {
	...
	*/
	...
	function editpengaturan(){
			...
			$this->db->where("field","color1");
			$this->db->update("setting",array("value"=>$color1));

			$this->db->where("field","color2");
			$this->db->update("setting",array("value"=>$color2));

			$this->db->where("field","colorbutton");
			$this->db->update("setting",array("value"=>$button));

			$this->db->where("field","colorbody");
			$this->db->update("setting",array("value"=>$body));
	...
	public function detail_produk($url=null){
		if(isset($url) AND $url != null){
			...
			$this->load->view("main/detail_produk",array("data"=>$db));
	...
	public function masuk_dulu($url = null){
		if(isset($url) AND $url != null){
			
			$data['url'] = $url;
			
			$this->load->view("main/masuk_dulu",$data);
	...
	public function beli_wa($url=null){
		if(isset($url) AND $url != null){
			...
			$this->load->view("main/beli_wa",array("data"=>$db));
	...
	public function form_beli_wa($berat_produk=null, $nama_produk=null, $harga_produk=null, $jml_beli=null, $total_bayar=null, $varian=null, $subvarian=null, $catatan=null){
		if(isset($berat_produk) AND $berat_produk != null){
			$nama_produk = str_replace("%20", " ", $nama_produk);
			$varian = str_replace("%20", " ", $varian);
			$subvarian = str_replace("%20", " ", $subvarian);
			$catatan = str_replace("%20", " ", $catatan);
			$totalberat = $jml_beli * $berat_produk;

			$data['totalberat'] = $totalberat;
			$data['berat_produk'] = $berat_produk;
			$data['nama_produk'] = $nama_produk;
			$data['harga_produk'] = $harga_produk;
			$data['jml_beli'] = $jml_beli;
			$data['total_bayar'] = $total_bayar;
			$data['varian'] = $varian;
			$data['subvarian'] = $subvarian;
			$data['catatan'] = $catatan;
			
			$this->load->view("main/form_beli_wa",$data);
	...
	public function masuk_yuk(){
		if ($_POST['email_otp'] !== 0) {
			$this->db->where("username",$_POST["email_otp"]);
			$this->db->or_where("nohp",$_POST["email_otp"]);
			$this->db->limit(1);
			$db = $this->db->get("userdata");
			$set = $this->func->getSetting("semua");
			...
			if($db->num_rows() == 0){
				...
				foreach($db->result() as $res){
					...
					$pesan = "
						<b>Perhataian</b><br/>".
						"Kode OTP untuk login ke akun".strtoupper(strtolower($set->nama)).", : <b>".$otp.
						"<br>Jangan berikan kode ini kepada siapa pun, termasik tim ".strtoupper(strtolower($set->nama))."<br/>".
						"WASPADA PENIPUAN!<br/>"
					;
					//$this->func->sendEmail($res->username,$set->nama." - OTP Login",$pesan,"OTP Login");
					$this->func->sendWAOK($this->func->getProfil($res->id,"nohp","usrid"),$pesan);
	...
	public function login_biasa(){
		...
		if(isset($_POST["email"])){
			...
			$aktif = false;
			if($db->num_rows() == 0){
				$pesan = "Email & No HP Tidak terdaftar";
				echo json_encode(array("success"=>false, "pesan"=>$pesan,"token"=>$this->security->get_csrf_hash()));
				exit;
			}
			foreach($db->result() as $res){
				$pass = $this->func->decode($res->password);
			...
			if($aktif == false){
				$pesan = "Email & No HP Belum diverifikasi";
				echo json_encode(array("success"=>false, "pesan"=>$pesan,"token"=>$this->security->get_csrf_hash()));
