...
class Assync extends CI_Controller {
	...
	function bayarpesanan(){
		if(isset($_POST["total"]) AND isset($_SESSION["usrid"])){
			...
			$status = ($_POST["kurir"] == "cod") ? 1 : $status;
				...
				$this->func->sendWAOK($profil->nohp,$pesan);
				$pesan = "
					...
					<a href='".site_url("i")."'>Klik Disini</a>
				";
				...
				$this->func->sendWAOK($toko->wasap,$pesan);
	...
	function bayarpesananwa(){
				$form_nama_penerima = $_POST['form_nama_penerima'];
				$totalsemua = $this->func->formUang($_POST['totalsemua']);
				$form_no_hp = $_POST['form_no_hp'];
				
				$form_nama_produk = $_POST['form_nama_produk'];
				$form_varian = $_POST['form_varian'];
				$form_subvarian = $_POST['form_subvarian'];
				$form_harga =  $this->func->formUang($_POST['form_harga']);
				$form_jml_beli = $_POST['form_jml_beli'];
				...
				$alamat_kab = $_POST['alamat_kab'];
				$alamat_kec = $_POST['alamat_kec'];
				$ongkir=  $this->func->formUang($_POST['ongkir']);
				$kodepos = $_POST['kodepos'];

				//informasi kurir
				$kurir_nama = $_POST['kurir_nama'];
				$kurir_service = $_POST['kurir_service'];
				...
				$pesan = "
					...
					"Produk: ".$form_nama_produk."\n".
					"Varian: ".$form_varian." | ".$form_subvarian."\n".
					"Harga: ".$form_harga." x ".$form_jml_beli."\n".
					"Ongkir: ".$ongkir."\n".
					"Total: ".$totalsemua."\n".
					"Catatan: ".$form_catatan."\n".
					"\n".
					"*Informasi Pengiriman*\n".
					"Nama Penerima: ".$form_nama_penerima."\n".
					"No HP: ".$form_no_hp."\n".
					"Alamat Rumah: ".$form_alamat."\n".
					"Provinsi: ".$alamat_prov."\n".
					"Kab/Kota: ".$alamat_kab."\n".
					"Kec: ".$alamat_kec."\n".
					"Kode Pos: ".$kodepos."\n".
					"Kurir: ".$kurir_nama." | ".$kurir_service."\n
				";
				foreach($bank->result() as $bn){
				$pesan .= "
					Bank: ".$bn->nama." No Rekening: ".$bn->norek." a/n ".$bn->atasnama."\n
				";
				}
				$pesan .= "\n";
				
				$pesan .= "Segera lakukan pembayaran & Kirimkan bukti pembayaran anda, pesanan anda akan segera kami proses. \n".
					"\n".
					"Terimakasih banyak sudah berbelanja di ".$toko->nama."\n".
					"Semoga Anda sekeluarga sehat dan sukses selalu\n".
					"\n".
					"Salam ".$toko->nama
				...
				$send = $this->func->sendWAOK($form_no_hp,$pesan);
				if ($send == true) {
					echo json_encode(array("success"=>true, "pesan"=>$pesan,"token"=>$this->security->get_csrf_hash()));
					exit;
				}else{
					echo json_encode(array("success"=>false, "pesan"=>"Gagal","token"=>$this->security->get_csrf_hash()));
					exit;
				}
	...
	public function getkab(){
		...
		foreach($db->result() as $res){
			$result .= "<option  value='".$res->id."' data-rajaongkir='".$res->rajaongkir."' data-nama='".$res->nama."'>".$res->tipe." ".$res->nama."</option>/n";
		}
	...
	public function getkec(){
		...
		foreach($db->result() as $res){
			$result .= "<option value='".$res->id."' data-rajaongkir='".$res->rajaongkir."' data-nama='".$res->nama."'>".$res->nama."</option>/n";
		}
		echo json_encode(array("rajaongkir"=>$rajaongkir,"html"=>$result,"token"=> $this->security->get_csrf_hash()));
	}
