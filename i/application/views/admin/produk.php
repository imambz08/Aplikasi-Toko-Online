...
<a href="<?=site_url("signin/produkform")?>" class="btn btn-success float-right m-r-10"><i class="fas fa-plus-circle"></i> Produk Baru</a>
...
<script type="text/javascript">
	$(function(){
		$.post("<?=site_url("signin/produk?load=true")?>&page=1",{"cari":$("#cari").val(),[$("#names").val()]:$("#tokens").val()},function(msg){
			var data = eval("("+msg+")");
		...
		$("#cari").change(function(){
			...
			$.post("<?=site_url("signin/produk")?>?page=1&load=true",{"cari":$("#cari").val(),[$("#names").val()]:$("#tokens").val()},function(msg){
				var data = eval("("+msg+")");
				updateToken(data.token);
				$(".card-body").html(data.result);
			});
	...
	function refreshTabel(page){
		$(".card-body").html('<i class="fas fa-spin fa-spinner"></i> Loading data...');
		$.post("<?=site_url("signin/produk?load=true")?>&page="+page,{"cari":$("#cari").val(),[$("#names").val()]:$("#tokens").val()},function(msg){
			var data = eval("("+msg+")");
			updateToken(data.token);
			$(".card-body").html(data.result);
		});
				...
				$.post("<?=site_url("api/hapusproduk")?>",{"id":id,[$("#names").val()]:$("#tokens").val()},function(msg){
					...
					if(data.success == true){
						swal.fire("Berhasil","data telah dihapus","success").then((val)=>{
							window.location.href="<?=site_url("signin/produk")?>";
						});
					}else{
